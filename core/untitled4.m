function [trldata,evt] = core_gettrldata(data_freq,ana,dir,exper)
...
for isub = 1:length(exper.subjects)
    ...
    for iconds = 1:length(fnames)
        ...
        for itrl = 1:length(trln)
            tidx(itrl) = find(trln(itrl)==evttrl);
        ...
        if ismember('stim',tempstruct.(fnames{iconds}).vars)
            ...
            tempstruct.(fnames{iconds}).vals(:,end) = acc_cor(tempstruct.(fnames{iconds}).vals(:,stimidx));
