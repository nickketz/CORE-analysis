function core_nk_ft_pow_probetest_wrapper(whichStages)
...
cfgfile = fullfile(dirs.saveDirProc, 'CORE_nk_ft_pow_probetest_wrapper_cfg.mat');
load(cfgfile,'cfg_gr');
...
for i = 1:length(STAGES)
    ...
    stageFun{i}(cfg_gr,ana,exper,dirs,adFile,runLocally,timeOut{i});
...
function stage1(cfg_gr,ana,exper,dirs,adFile,runLocally,timeOut)
...
%job = newJob(dirs); %run all subs at once
for isub = 1:length(cfg_gr.subs)
    job = newJob(dirs); %run one subs at a time
    
    cfg_gr.sub = cfg_gr.subs{isub};
    %create task for each latency interval to train classifier in
    %parallel
    fprintf('Running classifier training on %s\n',cfg_gr.sub);    
    
    for ilat = 1:length(cfg_gr.trainlatencies)
        cfg_gr.trainlatency = [cfg_gr.trainlatencies(ilat) cfg_gr.trainlatencies(ilat)+cfg_gr.latwidth];
        inArg = {cfg_gr,adFile};
        createTask(job,@core_probeTest,0,inArg);
    end
...
%destroy(job);
    ...
    %run one sub
    runJob(job, timeOut, fullfile(dirs.saveDirProc,[exper.name,'_CORE' cfg_gr.sub(end-1:end) '_stage1_',datestr(now,'ddmmmyyyy-HHMMSS'),'.log']));
    destroy(job);
end
...
function stage2(cfg_gr,ana,exper,dirs,adFile,runLocally,timeOut)

%load saved classifier performance
stats = nk_ft_read_train_data(cfg_gr,dirs,exper);
