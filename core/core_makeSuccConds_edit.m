function [exper,newdata] = core_makeSuccConds_edit(cfg,adFile)
...
for isub = 1:length(cfg.subjects)
    ...
    for icond = 1:length(conds)
        ...
        if sum(strcmp('succ',vars))==0
            ...
            newdata(icond).(cfg.param) = cat(1,newdata(icond).(cfg.param),data.(cfg.param)(samecond,:,:,:));
    ...
    if cfg.savedata ==1
        for icond = 1:length(conds)
            ...
            if ~isempty(data.(cfg.param))
                ...
                savefile = fullfile(savedir, ['data_' cfg.datatype '_' conds{icond} '_Succ.mat']);
