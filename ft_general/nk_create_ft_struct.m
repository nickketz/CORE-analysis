function [exper] = nk_create_ft_struct(ana,cfg_pp,cfg_proc,exper,dirs,files)
%CREATE_FT_STRUCT: Create a FieldTrip data structure from NS files
%NK edit: runs process_ft_data immediately after, added for parallel runs
%
...
for sub = 1:length(exper.subjects)
  for ses = 1:length(exper.sessions)
    ...
    elseif ~strcmpi(exper.eegFileExt,'set') && ~isempty(eventValuesToProcess)
      ...
      % get trial data from evt file
      trldata = core_gettrldata(ft_raw,dirs,exper,sub);
      if ~isfield(exper,'trldata')
          exper.trldata = trldata;
      else
          exper.trldata(sub) = trldata;
      end
