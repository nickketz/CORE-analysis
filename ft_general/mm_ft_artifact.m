...
%   %nk - 9.7.12
%   % edit to correct bad trial selection
%   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   % no sorting should be done
%   startTimeSorted = sesSummary{2};
%   badSorted = sesSummary{4};
%   eventValueSorted = sesSummary{1};
%   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if (rejArt_nsAuto || rejArt_zeroVar) && foundArt
  ...
  %verify trial counts match 'good' trials per condition
  for ievt = 1:length(eventValue)
      evtind = strcmp(eventValue{ievt},sesSummary{1});
      trlind = data.trialinfo(:,1) == ievt;
      if sum(strcmp('good',sesSummary{4}(evtind))) ~= sum(trlind)
          error('Remaining trials in condition %s don''t match ''good'' trials in bci file',eventValue{ievt});
      end
  end
