        ...
        if exist(inputfile,'file')
          ...
          if length(fn) == 1
            if keeptrials == 0 && strcmp(ftype,'pow') && isfield(subSesEvData.(cell2mat(fn)),'powspctrm') && ndims(subSesEvData.(cell2mat(fn)).powspctrm) == 4
              ...
              %%%% 
              % 10.11.12 nk 
              % fix for no trials in a conditoin
              %%%%
              if size(subSesEvData.freq.powspctrm,1) == 0
                  fprintf('\nNo trials found! skipping averaging and returning empty powspctrm matrix');
            ...
            error('More than one field in data struct! There can be only one.\n');
          end
