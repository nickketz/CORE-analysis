function [ft_raw,badChanAllSes] = nk_seg2ft(dataroot,subject,session,eventValue,elecfile,ana,exper)
%SEG2FT: take segmented EEG data and put it in FieldTrip format
%
% [ft_raw,badChan] = nk_seg2ft(dataroot,subject,session,eventValue,elecfile,ana,exper)
%
% Output:
...
for ses = 1:length(session)
  ...
  %find evt file for this subject
  evtFile = dir(fullfile(dataroot,sesName,'ns_evt',[subject,'*.evt']));
  if ~isempty(evtFile)
      evtFile = fullfile(dataroot,sesName,'ns_evt',evtFile.name);
      %read in evt file
      cfg.evt = readevt(evtFile,exper.sampleRate);
      %filter evt file based on particular experiment (core in this case)
      cfg.evt = core_filtevt(cfg.evt);
  else
      error('evt file for subject %s not found!\n',subject);
  end
...
%   %add trldata to ana struct
  ...
  %% include evt meta data in cfg.trailinfo
  
%   evtFile = dir(fullfile(dataroot,sesName,'ns_evt',[subject,'*.evt']));
%   if ~isempty(evtFile)
%       evtFile = fullfile(dataroot,sesName,'ns_evt',evtFile.name);
%       evt = readevt(evtFile,exper.sampleRate);
%       trialinfo = zeros(length(cfg.trl),1);
%       linind = 0;
%       for icat = 1:length(eventValue)
%           for itrl = 1:length(find(cfg.trl(:,end)==icat))
%               linind = linind+1;
%               %add trial number to trial info in cfg
%               trialinfo(linind,:) = evt.(eventValue{icat}).vals(itrl,2);
%           end
%       end
%       cfg.trl(:,end) = [cfg.trl trialinfo];            
%   end
%       
%       
...
if length(eventValue) > 1
  for evVal = 1:length(eventValue)
    ...
    cfg.trials = data.trialinfo(:,1) == evVal;
